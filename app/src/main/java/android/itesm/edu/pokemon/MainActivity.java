            ...
            public void onResponse(JSONObject response) {
                try {
                    ...
                    for (int i = 0; i < cards_json.length(); i++) {
                        obj = cards_json.getJSONObject(i);
                        PokeCard pokeCard = new PokeCard();
                        pokeCard.setId(obj.getString("id"));
                        pokeCard.setArtist(obj.getString("artist"));
                        pokeCard.setImageUrl(obj.getString("imageUrl"));
                        pokeCard.setName(obj.getString("name"));
                        cards.add(pokeCard);
                    }
                }
